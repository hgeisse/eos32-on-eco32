...
void readBlock(int dskfd,
               EOS32_daddr_t blockNum,
               unsigned char *blockBuffer) {
  lseek(dskfd, blockNum * BLOCK_SIZE, SEEK_SET);
  if (read(dskfd, blockBuffer, BLOCK_SIZE) != BLOCK_SIZE) {
...
int waitForReturn(void) {
  ...
  printf("press <enter> to continue, b to break, q to quit: ");
  fflush(stdout);
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    printf("Usage: %s <disk or partition special file>\n", argv[0]);
    exit(1);
  }
  dskfd = open(argv[1], O_RDONLY);
  if (dskfd < 0) {
    error("cannot open disk or partition special file '%s'", argv[1]);
  }
  currBlock = 1;
  readBlock(dskfd, currBlock, blockBuffer);
  help();
  quit = 0;
  while (!quit) {
