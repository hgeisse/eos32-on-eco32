...
int dskfd;		/* the file descriptor of the disk or partition */
...
void rdfs(EOS32_daddr_t bno, unsigned char *bf, int blkType) {
  ...
  lseek(dskfd, bno * BSIZE, SEEK_SET);
  n = read(dskfd, bf, BSIZE);
  if (n != BSIZE) {
...
int main(int argc, char *argv[]) {
  ...
  time(&now);
  fsnam = argv[1];
  dskfd = open(fsnam, O_RDWR);
  if (dskfd < 0) {
    error("cannot open disk or partition '%s'", fsnam);
  }
  proto = argv[2];
  ...
  /* note: this isn't strictly necessary, but cuts down file
     creation time within the newly created disk or partition */
  fillFreeInodeList(numInodes);
  /* finally write super block, close disk or partition, and return */
