...
/*	IF c>0 ANDF any('r', *v) THEN rflag=0 FI */
	...
	THEN	/* ? profile */
		...
		ELSE	input=((flags&stdflg) ? 0 : chkopen(cmdadr));
			comdiv--;
		FI
	ELSE	; /* HG: zapped -- *execargs=dolv;*/	/* for `ps' cmd */
	FI
...
LOCAL VOID	exfile(BOOL prof)
{
	...
	IF (flags&intflg) ORF ((flags&oneflg)==0 ANDF gtty(output,&ttyb)==0 ANDF gtty(input,&ttyb)==0)
...
VOID settmp(VOID)
{
	...
	tmpname=movstr(numbuf,&tmpout[TMPNAM]);
}
