...
INT execute(TREPTR argt, INT execflg, INT *pf1, INT *pf2)
{
	...
	THEN	REG INT		treeflgs;
		...
		treeflgs = t->pfn.tretyp; type = treeflgs&COMMSK;
		...
		case TCOM:
			...
			THEN	/* print command if execpr */
				...
				case SYSTIMES:
					{
					L_INT	t[4]; times((struct tbuffer *)t);
...
*/
		...
		case TFORK:
			...
			ELSE	/* this is the forked branch (child) of execute */
				...
				oldsigs();
				IF treeflgs&FINT
				THEN	signal(INTR,(VOID (*)(INT))1);
					signal(QUIT,(VOID (*)(INT))1);
				FI

				/* pipe in or out */
				IF treeflgs&FPIN
				THEN	renam(pf1[INPIPE],0);
					close(pf1[OTPIPE]);
				FI
				IF treeflgs&FPOU
				THEN	renam(pf2[OTPIPE],1);
					close(pf2[INPIPE]);
				FI

				/* default std input for & */
				IF treeflgs&FINT ANDF ioset==0
				THEN	renam(chkopen(devnull),0);
				FI

				/* io redirection */
				initio(t->pfn.treio);
				IF type!=TCOM
				THEN	execute(t->forkn.forktre,1,0,0);
				ELIF com[0]!=ENDARGS
				THEN	setlist(t->comn.comset,N_EXPORT);
					execa(com);
				FI
				done();
			FI
		...
		case TPAR:
			renam(dup(2),output);
			execute(t->parn.partre,execflg,0,0);
			done();

		case TFIL:
			BEGIN
			   INT pv[2]; chkpipe(pv);
			   IF execute(t->lstn.lstlef, 0, pf1, pv)==0
			   THEN	execute(t->lstn.lstrit, execflg, pv, pf2);
			   ELSE	closepipe(pv);
			   FI
			END
			break;
		...
		case TLST:
			execute(t->lstn.lstlef,0,0,0);
			execute(t->lstn.lstrit,execflg,0,0);
			break;

		case TAND:
			IF execute(t->lstn.lstlef,0,0,0)==0
			THEN	execute(t->lstn.lstrit,execflg,0,0);
			FI
			break;
		...
		case TFOR:
			BEGIN
			   ...
			   ELSE	   ARGPTR	schain=gchain;
				   gchain=0;
				   trim((args=scan(getarg(t->forn.forlst)))[0]);
				   gchain=schain;
			   FI
			   loopcnt++;
			   WHILE *args!=ENDARGS ANDF execbrk==0
			   DO	assign(n,*args++);
				execute(t->forn.fortre,0,0,0);
		...
		case TUN:
			BEGIN
			   ...
			   loopcnt++;
			   WHILE execbrk==0 ANDF (execute(t->whn.whtre,0,0,0)==0)==(type==TWH)
			   DO i=execute(t->whn.dotre,0,0,0);
			      IF execbrk<0 THEN execbrk=0 FI
			   OD
			   IF breakcnt THEN breakcnt-- FI
		...
		case TIF:
			IF execute(t->ifn.iftre,0,0,0)==0
			THEN	execute(t->ifn.thtre,execflg,0,0);
			ELSE	execute(t->ifn.eltre,execflg,0,0);
			FI
			break;

		case TSW:
			BEGIN
			   REG STRING	r = mactrim(t->swn.swarg);
			   t=(TREPTR)(t->swn.swlst);
			   WHILE t
			   DO	ARGPTR		rex=t->regn.regptr;
				WHILE rex
				DO	REG STRING	s;
					IF gmatch(r,s=macro(rex->argval)) ORF (trim(s), eq(r,s))
					THEN	execute(t->regn.regcom,0,0,0);
						t=0; break;
					ELSE	rex=rex->argnxt;
					FI
				OD
				IF t THEN t=(TREPTR)(t->regn.regnxt) FI
			   OD
			END
			break;
		ENDSW
