/*
 * shpart.c -- show partitions on a disk
 */


#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdarg.h>
#include <eos32.h>


#define SECTOR_SIZE	512
#define NPE		(SECTOR_SIZE / sizeof(PartEntry))
#define DESCR_SIZE	20
#define PART_MAGIC	0xF5A5F2F9


typedef struct {
  unsigned long type;
  unsigned long start;
  unsigned long size;
  char descr[DESCR_SIZE];
} PartEntry;

PartEntry ptr[NPE];

/*
 * NOTE: The last entry ptr[NPE - 1] is not used for
 * any real partition, but holds the magic number which
 * identifies the ECO32 partitioning scheme. The struct
 * members 'type' and 'size' must each contain PART_MAGIC,
 * and 'start' must contain the bitwise complement of it.
 */


/**************************************************************/


void error(char *fmt, ...) {
  va_list ap;

  va_start(ap, fmt);
  printf("Error: ");
  vprintf(fmt, ap);
  printf("\n");
  va_end(ap);
  exit(1);
}


/**************************************************************/


unsigned long getNumber(unsigned char *p) {
  return (unsigned long) *(p + 0) << 24 |
         (unsigned long) *(p + 1) << 16 |
         (unsigned long) *(p + 2) <<  8 |
         (unsigned long) *(p + 3) <<  0;
}


void convertPartitionTable(PartEntry *e, int n) {
  int i;
  unsigned char *p;

  for (i = 0; i < n; i++) {
    p = (unsigned char *) &e[i];
    e[i].type = getNumber(p + 0);
    e[i].start = getNumber(p + 4);
    e[i].size = getNumber(p + 8);
  }
}


/**************************************************************/


int main(int argc, char *argv[]) {
  char *diskName;
  int dskfd;
  unsigned long partLast;
  int i, j;
  char c;
  char machineMode = 0;

  /* check command line arguments */
  
  if (argc < 2 || argc > 3) {
    printf("Usage: %s <disk special file> [--readable-for-mkpart]\n", argv[0]);
    exit(1);
  }
  diskName = argv[1];
  if(argc == 3 && (! strcmp(argv[2], "--readable-for-mkpart"))) {
    machineMode = 1;
  }
  /* read partition table record */
  dskfd = open(diskName, O_RDONLY);
  if (dskfd < 0) {
    error("cannot open disk '%s'", diskName);
  }
  lseek(dskfd, 1 * SECTOR_SIZE, SEEK_SET);
  if (read(dskfd, ptr, SECTOR_SIZE) != SECTOR_SIZE) {
    error("cannot read partition table from disk '%s'", diskName);
  }
  close(dskfd);
  convertPartitionTable(ptr, NPE);
  /* check magic number */
  if (ptr[NPE - 1].type != PART_MAGIC ||
      ptr[NPE - 1].start != ~PART_MAGIC ||
      ptr[NPE - 1].size != PART_MAGIC) {
    error("wrong magic number in partition table");
  }
  /* show partition table */
  if(!machineMode) {
    /* human readable output */
    printf("Partitions:\n");
    printf(" # b type       start      last       size       description\n");
    for (i = 0; i < NPE - 1; i++) {
      if (ptr[i].type != 0) {
        partLast = ptr[i].start + ptr[i].size - 1;
      } else {
        partLast = 0;
      }
      printf("%2d %s 0x%08lX 0x%08lX 0x%08lX 0x%08lX ",
            i,
            ptr[i].type & 0x80000000 ? "*" : " ",
            ptr[i].type & 0x7FFFFFFF,
            ptr[i].start,
            partLast,
            ptr[i].size);
      for (j = 0; j < DESCR_SIZE; j++) {
        c = ptr[i].descr[j];
        if (c == '\0') {
          break;
        }
        if (c >= 0x20 && c < 0x7F) {
          printf("%c", c);
        } else {
          printf(".");
        }
      }
      printf("\n");
    }
  } else {
    /* machine readable output */
    printf("# auto-generated by shpart\n");
    printf("# nr b type start end desc\n");
    for (i = 0; i < NPE - 1; i++) {
      if(ptr[i].size == 0) continue; /* skip empty partitions */
      if (ptr[i].type != 0) {
        partLast = ptr[i].start + ptr[i].size - 1;
      } else {
        partLast = 0;
      }
      printf("%-2d %s 0x%08lX 0x%08lX 0x%08lX ",
            i,
            ptr[i].type & 0x80000000 ? "*" : " ",
            ptr[i].type & 0x7FFFFFFF,
            ptr[i].start,
            partLast);
      printf("\"");
      for (j = 0; j < DESCR_SIZE; j++) {
        c = ptr[i].descr[j];
        if (c == '\0') {
          break;
        }
        if (c >= 0x20 && c < 0x7F) {
          printf("%c", c);
        } else {
          printf(".");
        }
      }
      printf("\"");
      printf("\n");
    }
  }
  /* done */
  return 0;
}
