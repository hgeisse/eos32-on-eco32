...
struct f_f {POS int r; unsigned int x; unsigned int y; unsigned int dy; int e; };
struct l_l {POS int r; long double x; long double y; float dy; int e; };
struct ff_f {POS int r; unsigned int x; unsigned int x2; unsigned int y; unsigned int dy; int e; };
...
static int checkulp(float d, int r)
{
  ...
  if (r == RN) {
    /*
     * bfranken TODO remove debugging
    printf("--->checkulp<--- bfranken DEBUG d = %f\n", d);
    printf("--->checkulp<--- bfranken DEBUG fabsf(d) = %f\n", fabsf(d));
    printf("--->checkulp<--- bfranken DEBUG returning %d\n", fabsf(d) < 1.5);
    */
    return fabs(d) < 1.5;
  }
