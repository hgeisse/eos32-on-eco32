...
void aopen(void);	/* fflush(stdout) and then set terminal to
			   character at a time mode */
void aclose(void);	/* Flush output and then restore original tty mode */
void aflush(void);	/* Flush the output and sleep for the amount of time
...
int anext(void);	/* Call aflush() and then return next char from
			   terminal */
void sigjoe(void);	/* Set signal handling for JOE */
void signorm(void);	/* Set signal handling back to default */

extern int have;	/* Set if there is typeahead */
extern int leave;	/* Set to prevent typehead checking */

void eputs(char *s);	/* Write string to terminal */
void eputc(int c);	/* Write character to terminal */
			/* If the output buffer gets full, these call
			   aflush() */

void shell(void);	/* Shell escape */
void susp(void);	/* Suspend */

void termtype(void);	/* Determine the following terminal parameters */
...
void getsize(void);	/* Set width and height again with TIOCGSIZE */

/* If termtype does nothing, these values default to: width=80, height=24,
   scroll=1 (true).
