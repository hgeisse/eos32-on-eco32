/*
 * txt.c -- handle shared text segments
 */
...
void xswap(struct proc *p, int ff, int os)
{
	...
	if(os == 0)
		os = p->p_size;
	a = malloc(swapmap, PAGES2BLOCKS(p->p_size));
	if(a == -1)
		panic("out of swap space");
	p->p_flag |= SLOCK;
	xccdec(p->p_textp);
...
void xfree(void)
{
	...
	if(--xp->x_count==0 && (ip->i_mode&ISVTX)==0) {
		xp->x_iptr = NULL;
		mfree(swapmap, PAGES2BLOCKS(xp->x_size), xp->x_daddr);
...
{
	if ((xp->x_caddr = malloc(coremap, xp->x_size)) != -1) {
