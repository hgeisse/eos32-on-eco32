...
void stat1(struct inode *ip, struct stat *ub, off_t pipeadj)
{
	...
	ds.st_uid = ip->i_uid;
	ds.st_gid = ip->i_gid;
	ds.st_rdev = (dev_t)ip->i_un.i_s2.i_rdev;
...
void smount(void)
{
	...
	if (bp->b_un.b_filsys->s_magic != SUPER_MAGIC) {
		/* wrong magic number - bad superblock */
		brelse(bp);
		u.u_error = EINVAL;
		goto out1;
	}
