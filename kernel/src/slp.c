...
Bool debugWakeup = FALSE;
Bool debugSetpri = FALSE;
Bool debugSched = FALSE;
Bool debugSwtch = FALSE;
Bool debugNewproc = FALSE;
Bool debugExpand = FALSE;
...
/*
 * Show all non-free entries of the process table.
 */
void showProcTable(char *header) {
  ...
  printf("slot  state   flags      pid   ppid   addr   size\n");
  for (i = 0; i < NPROC; i++) {
    p = &proc[i];
    if (p->p_stat == SFREE) {
      continue;
    ...
    printf("%4d", i);
    printf("  ");
    printf("%s", stateName[p->p_stat]);
    printf("  ");
    printf("%c", p->p_flag & SULOCK ? 'U' : '-');
    printf("%c", p->p_flag & SWTED ? 'W' : '-');
    printf("%c", p->p_flag & STRC ? 'T' : '-');
    printf("%c", p->p_flag & SSWAP ? 'S' : '-');
    printf("%c", p->p_flag & SLOCK ? 'K' : '-');
    printf("%c", p->p_flag & SSYS ? 'Y' : '-');
    printf("%c", p->p_flag & SLOAD ? 'L' : '-');
    printf("  ");
    printf("%5d", p->p_pid);
    printf("  ");
    printf("%5d", p->p_ppid);
  ...
  printf("(flags = ULOCKED | WAITED | TRACED | ");
  printf("SWAPPED | LOCKED | SYSTEM | LOADED)\n");
  printf("\n");
}
...
void sleep(caddr_t chan, int pri)
{
  ...
  if (debugSleep) {
    printf("-----  sleep (chan = 0x%08X, pri = %d)  -----\n", chan, pri);
  }
...
int swapin(struct proc *p)
{
	...
	if ((a = malloc(coremap, p->p_size)) == -1)
		return(0);
	if (xp = p->p_textp) {
		xlock(xp);
		if (xp->x_ccount==0) {
			if ((x = malloc(coremap, xp->x_size)) == -1) {
				xunlock(xp);
	...
	mfree(swapmap, PAGES2BLOCKS(p->p_size), p->p_addr);
	p->p_addr = a;
	p->p_flag |= SLOAD;
	p->p_time = 0;
	return(1);
