...
{
	tun.t_tc.t_intrc = (char) CINTR;
	tun.t_tc.t_quitc = (char) CQUIT;
	tun.t_tc.t_startc = (char) CSTART;
	tun.t_tc.t_stopc = (char) CSTOP;
	tun.t_tc.t_eofc = (char) CEOT;
	tun.t_tc.t_brkc = (char) CBRK;
	tp->t_erase = CERASE;
	tp->t_kill = CKILL;
}
...
loop:
	...
	if (tp->t_state&TBLOCK && tp->t_rawq.c_cc < TTYHOG/5) {
		if (putc(tun.t_tc.t_startc, &tp->t_outq)==0) {
			tp->t_state &= ~TBLOCK;
			ttstart(tp);
		}
		tp->t_char = 0;
...
void ttyinput(int c, struct tty *tp)
{
	...
	if ((t_flags&RAW)==0) {
		...
		if (tp->t_state&TTSTOP) {
			if (c==tun.t_tc.t_startc) {
				tp->t_state &= ~TTSTOP;
				ttstart(tp);
				return;
			}
			if (c==tun.t_tc.t_stopc)
				return;
			tp->t_state &= ~TTSTOP;
			ttstart(tp);
		} else {
		...
		if (c==tun.t_tc.t_quitc || c==tun.t_tc.t_intrc) {
			flushtty(tp);
			c = (c==tun.t_tc.t_intrc) ? SIGINT:SIGQUIT;
			if (tp->t_chan) {
				/* HG: multiplexor stuff no longer present */
...
register x;
	...
	if (x >= TTYHOG/2) {
		if (putc(tun.t_tc.t_stopc, &tp->t_outq)==0) {
			tp->t_state |= TBLOCK;
			tp->t_char++;
			ttstart(tp);
		}
